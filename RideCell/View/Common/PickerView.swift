...
//  boAt Wearables
//
//  Created by neelkant on 21/09/20.
...
@objc public protocol AKPickerViewDataSource {
    ...
    @objc optional func pickerView(_ pickerView: AKPickerView, carForItem item: Int) -> CarDetail
...
//            contentView.backgroundColor = self.isSelected ? .red  : .clear
...
public class AKPickerView: UIView, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, AKCollectionViewLayoutDelegate {
    ...
    public  var redViewHeight : CGFloat = 120 {
        didSet {
//            centerViewWidthConstraint.constant = redViewHeight
//            centerView.layer.cornerRadius = redViewHeight / 2
        }
    }
    
    /// Readwrite. A font which used in NOT selected cells.
    public lazy var font = UIFont.systemFont(ofSize: 20)
    public lazy var subFont = UIFont.systemFont(ofSize: 14)

    /// Readwrite. A font which used in selected cells.
    public lazy var highlightedFont = UIFont.systemFont(ofSize: 32)
    /// Readwrite. A color of the text on NOT selected cells.
    @IBInspectable public lazy var textColor: UIColor = .systemRed

    /// Readwrite. A color of the text on selected cells.
    @IBInspectable public lazy var highlightedTextColor: UIColor = UIColor.white
    ...
    */
    public func reloadData() {
        ...
        guard  let dataSourceCount = self.dataSource?.numberOfItemsInPickerView(self) else {
            return
        }
        if self.dataSource != nil && dataSourceCount > 0 {
           if let firstTitle = self.dataSource?.pickerView?(self, titleForItem: 0) {
             var width = min(100,max(90,sizeForString(firstTitle as NSString).width + 25)) 
            if firstTitle.count > 3 {
               width += 10
            }
            }
            if self.selectedItem < dataSourceCount {
    ...
    */
    public func scrollToItem(_ item: Int, animated: Bool = false) {
        guard  let dataSourceCount = self.dataSource?.numberOfItemsInPickerView(self), item < dataSourceCount else {
            return
        }
    ...
    :param: animated True if the scrolling should be animated, false if it should be immediate.
    */
    public func selectItem(_ item: Int, animated: Bool = false , name : String = #function) {
    ...
    */
    fileprivate func selectItem(_ item: Int, animated: Bool, notifySelection: Bool, name : String = #function) {
        self.collectionView.selectItem(at: IndexPath(item: item, section: 0), animated: animated, scrollPosition: .centeredHorizontally)
    ...
    */
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let car = self.dataSource?.pickerView?(self, carForItem: indexPath.item) {
            let carCell = collectionView.dequeueReusableCell(withReuseIdentifier: CarsCollectionViewCell.identifier, for: indexPath) as! CarsCollectionViewCell
            carCell.configureCell(car: car)
            return carCell
        }
    ...
    public func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        var size = CGSize(width: self.interitemSpacing + 200, height: collectionView.bounds.size.height)
        return size
    }
